...
export function ReviewWorkspace() {
  ...
  return <main className="review-shell">
    ...
    {state === "ready" && queue && <>
      <section className="review-summary"><article><small>PLACE DECISIONS</small><strong>{queue.places.length}</strong><span>pending or overdue</span></article><article><small>DEAL DECISIONS</small><strong>{queue.deals.length}</strong><span>submitted, expired, or overdue</span></article><article><small>AUDIT EVENTS</small><strong>{queue.events.length}</strong><span>latest 100 loaded</span></article><button onClick={() => void refresh()}>Refresh queue ↻</button></section>
